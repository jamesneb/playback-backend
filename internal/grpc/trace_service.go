	...
	for _, resourceSpan := range req.ResourceSpans {
		event := &streaming.TelemetryEvent{
			...
			Metadata: streaming.TelemetryMetadata{
				IngestedAt: time.Now(),
				SourceIP:   "grpc-client", // Could extract from gRPC metadata
			},
		}
...
func convertResourceSpanToMap(resourceSpan *tracepb.ResourceSpans) interface{} {
	// Convert protobuf to map structure that our existing parsing logic can handle
	// This allows reuse of the existing ClickHouse insertion code
	return map[string]interface{}{
		"resourceSpans": []interface{}{resourceSpan},
	}
}
